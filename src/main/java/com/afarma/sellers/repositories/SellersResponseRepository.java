...
public interface SellersResponseRepository extends JpaRepository<SellersResponse, SellersResponseId> {
    ...
    @Query(value = "SELECT A.trading_name, B.description, B.stock, B.active , B.price, date_register, seller_id,availability,id, '' as company_name, B.type, B.unidade_medida, B.detalhes, B.url_imagem " +
            ...
            "AND B.description LIKE :description% ", nativeQuery = true)
    ...
    @Query(value = "SELECT A.trading_name, B.description, B.stock, B.active , B.price, date_register, seller_id,availability,id, '' as company_name, B.type, B.unidade_medida, B.detalhes, B.url_imagem " +
            ...
            "UNION " +
            "SELECT description,  '-' as stock , active, price, date_register, seller_id, case when availability = 0 then 'NÃO' ELSE 'SIM' end,service_id, 'SER' as type, '-' as unidade_medida, '-' as detalhes, '-' as url_imagem FROM `afarma-sellers`.services) B " +
            "ON A.id = B.seller_id WHERE  COALESCE(NULLIF(:active,''), active) = active AND  date_register BETWEEN DATE_FORMAT(:dateIni,\"%Y-%m-%d\") AND DATE_FORMAT(:dateFin,\"%Y-%m-%d\") ", nativeQuery = true)
    Page<SellersResponse> findProductsAndSellersWithoutLike(Pageable pageable, @Param("active") Boolean active, @Param("dateIni") String dateIni,
                                                 @Param("dateFin") String dateFin);
    ...
    @Query(value = "SELECT A.trading_name, B.description, B.stock, B.active , B.price, date_register, seller_id,availability,id, '' as company_name, B.type,  B.unidade_medida, B.detalhes, B.url_imagem " +
            ...
            "UNION " +
            "SELECT description,  '-' as stock , active, price, date_register, seller_id, case when availability = 0 then 'NÃO' ELSE 'SIM' end,service_id, 'SER' as type, '-' as unidade_medida, '-' as detalhes, '-' as url_imagem FROM `afarma-sellers`.services) B " +
            "ON A.id = B.seller_id WHERE  active = false AND  date_register BETWEEN DATE_FORMAT(:dateIni,\"%Y-%m-%d\") AND DATE_FORMAT(:dateFin,\"%Y-%m-%d\") ", nativeQuery = true)
    Page<SellersResponse> findProductsAndSellersWithoutLikeFalse(Pageable pageable,@Param("dateIni") String dateIni, @Param("dateFin") String dateFin);
